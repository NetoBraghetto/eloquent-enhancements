...
trait SaveAll
{
    ...
    private function shouldUseSync($relationship, $data)
    {
        ...
        if ($relationship instanceof BelongsToMany && count($data) === 1) {
            // check foreign key
            $foreignKey = last(explode('.', $relationship->getRelatedKeyName()));
    ...
    private function checkBelongsTo($data, array $options = [], $path = '') {
        ...
        foreach ($relationships as $relationship => $values) {
            ...
            if (!$object->createAll($values, $options)) {
                ...
                $this->$foreignKey = $object->getPrimaryKeyValue();
            }
    ...
    public function addRelated($relationshipName, array $values, array $options = [], $path = '')
    {
        ...
        if ($relationship instanceof MorphMany) {
            $values[$relationship->getForeignKeyName()] = $this->getPrimaryKeyValue();
            $values[$relationship->getMorphType()] = get_class($this);
        }

        // if BelongsToMany, put current id in place
        if ($relationship instanceof BelongsToMany) {
            $values[last(explode('.', $relationship->getParentKeyName()))] = $this->getPrimaryKeyValue();
            $belongsToManyOtherKey = last(explode('.', $relationship->getRelatedKeyName()));
        }
        ...
        // only BelongsToMany :)
        if (!empty($values['_delete'])) {
            $pkName = $relationship->getRelated()->getPrimaryKeyName();
            $this->$relationshipName()->detach($values[$pkName]);
            return true;
        }
        ...
        if ($relationship instanceof HasMany || $relationship instanceof MorphMany) {
            ...
            if (empty($this->relationshipsModels[$relationshipName])) {
                $field = last(explode('.', $relationship->getRelatedKeyName()));
                $this->$relationshipName()->attach($values[$field]);
                return true;
            }

            $relationshipObjName = $this->relationshipsModels[$relationshipName];
            $relationshipObject = new $relationshipObjName;
        } elseif ($relationship instanceof HasManyThrough) {
            $relationshipObject = $model;
        }

        if (!empty($values[$relationshipObject->getPrimaryKeyName()])) {
            $relationshipObject = $relationshipObject->find($values[$relationshipObject->getPrimaryKeyName()]);
            if (!$relationshipObject) {
                return false; // @todo transport error
            }
            if (!$relationshipObject->saveAll($values, $options)) {
                $this->mergeErrors($relationshipObject->errors()->toArray(), $path);
                return false;
            }
        } else {
            if (!$relationshipObject->createAll($values, $options)) {
                $this->mergeErrors($relationshipObject->errors()->toArray(), $path);
                return false;
            }
        }
